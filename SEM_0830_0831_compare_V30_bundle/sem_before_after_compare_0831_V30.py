...
def _slot_four_row_array_v18(
    ...
    allow_diag=False
    print(f"      [{image_name}] slot four-row model: pool={len(pool)}, strict_seeds={len(strict_seed)}", flush=True)
    relaxed=[d for d in pool if _slot_shape_ok_v17(d,area_limits,seed=True)]
    model_cands=_dedup_desc_list_v16(strict_seed+relaxed)
    ...
    if row_model is None or len(model_cands)<8:
        print(f"      [{image_name}] slot sparse seeds={len(model_cands)} -> low-gray localization", flush=True)
        lowgray_loc=_slot_lowgray_model_proposals_v18(gray,image_name)
        ...
        print(f"      [{image_name}] slot low-gray proposals={len(lowgray_loc)}, fitting_candidates={len(fit_cands)}", flush=True)
        row_model=_fit_four_row_centers_v17(fit_cands,H,image_name)
    meta={
