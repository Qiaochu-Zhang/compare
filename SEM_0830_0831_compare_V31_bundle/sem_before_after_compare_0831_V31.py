...
# -*- coding: utf-8 -*-
"""
SEM before/after comparison V31: V27 geometry, Yakun edge evidence, V30 recovery and 300s image limit.
...
# Per user requirement these defaults are 1/10 and 10x.
SLOT_USE_RELATIVE_AREA_FILTER = True
SLOT_AREA_MIN_MEAN_FACTOR = 0.10
SLOT_AREA_MAX_MEAN_FACTOR = 10.0
...
def _fit_four_row_centers_v17(
    ...
    # V31: keep every original hypothesis and the original score/tie ordering.
...
V18_SLOT_LOWGRAY_MODEL_AREA_MAX_FRAC = 0.035
...
_V31_SLOT_RECOVERY_DIAGNOSTICS = Counter()


def _v31_slot_flatfield_response(roi: np.ndarray) -> Tuple[np.ndarray, float]:
...
def _v31_slot_flatfield_candidates(gray: np.ndarray, pool: List[ShapeDesc]) -> List[ShapeDesc]:
    ...
    for size in kernels:
        ...
        for fraction in (0.50, 0.35):
            ...
            candidates = extract_loose_descriptors_v12(mask, 'slot_v31_flatfield', 'slot', 160)
            for d in candidates:
...
def _v31_recover_flatfield_slot(
    ...
    if min(roi.shape, default=0) < 9:
        _V31_SLOT_RECOVERY_DIAGNOSTICS['roi_too_small'] += 1
        return None
    response, noise = _v31_slot_flatfield_response(roi)
    peak = float(np.percentile(response, 99.0))
    if peak < max(2.0, 3.0 * noise):
        _V31_SLOT_RECOVERY_DIAGNOSTICS['weak_local_contrast'] += 1
        return None
    ...
    for fraction in (0.50, 0.35, 0.65):
        ...
        contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        if not contours:
            _V31_SLOT_RECOVERY_DIAGNOSTICS['no_contour'] += 1
        for contour in contours:
            if contour_pixel_count(contour) < SLOT_MIN_AREA_PX:
                continue
            bx, by, bw, bh = cv2.boundingRect(contour)
            if bx <= 1 or by <= 1 or bx + bw >= roi.shape[1] - 1 or by + bh >= roi.shape[0] - 1:
                _V31_SLOT_RECOVERY_DIAGNOSTICS['touches_roi_border'] += 1
                continue
            ...
            if abs(float(d.center[0]) - cx) > xt or abs(float(d.center[1]) - cy) > yt:
                _V31_SLOT_RECOVERY_DIAGNOSTICS['outside_predicted_site'] += 1
                continue
            if not _slot_shape_ok_v17(d, area_limits, seed=False):
                _V31_SLOT_RECOVERY_DIAGNOSTICS['area_or_shape_qc'] += 1
                continue
    ...
    if best is None:
        return None
    _V31_SLOT_RECOVERY_DIAGNOSTICS['flatfield_recovered'] += 1
    return _mark_source(best[1], 'slot_v31_flatfield_recovered')


def _recover_local_slot_v18_deep(
    gray, expected_xy, x_pitch, row_pitch, expected_w, expected_h, area_limits, aggressive_level,
) -> Optional[ShapeDesc]:
    d = _v31_recover_flatfield_slot(gray, expected_xy, x_pitch, row_pitch, expected_w, expected_h, area_limits)
    if d is not None:
        return d
    d = _recover_local_slot_v18_legacy(gray, expected_xy, x_pitch, row_pitch, expected_w, expected_h, area_limits, aggressive_level)
    _V31_SLOT_RECOVERY_DIAGNOSTICS['legacy_recovered' if d is not None else 'site_not_recovered'] += 1
    return d
...
V21_VIA_LEGACY_MIN_DIAM_FACTOR = 0.62
V21_VIA_LEGACY_MAX_DIAM_FACTOR = 1.60
...
def _via_scale_response_v21(
    ...
    return score, best_raw, best_diam, {
        "nominal_diameter_px": float(nominal_diam_px),
...
V20_TRENCH_GAP_MAD_FACTOR = 5.0
V20_TRENCH_GAP_FRAC_TOL = 0.34
V20_TRENCH_WIDTH_MAD_FACTOR = 5.0
V20_TRENCH_WIDTH_FRAC_TOL = 0.55
...
# ============================================================
V23_VIA_MIN_SPAN_FACTOR = 0.72
V23_VIA_MAX_SPAN_FACTOR = 1.50
V23_VIA_MIN_EQ_DIAM_FACTOR = 0.74
V23_VIA_MAX_EQ_DIAM_FACTOR = 1.48
...
def _select_and_measure_vias_v19(
    ...
    for d in pre:
        ...
        for cc in unique:
            ...
            if min(spanx,spany)<0.66*D or max(spanx,spany)>1.58*D: continue
    ...
    for idx,d in enumerate(survivors):
        ...
        if not (V23_VIA_MIN_SPAN_FACTOR*nominal_nm<=w_nm<=V23_VIA_MAX_SPAN_FACTOR*nominal_nm and V23_VIA_MIN_SPAN_FACTOR*nominal_nm<=h_nm<=V23_VIA_MAX_SPAN_FACTOR*nominal_nm):
            rejects.append(_make_reject_from_contour(d.contour,'via_v24_physical_qc','via_final_span_out_of_range',f'actual X/Y spans {w_nm:.2f}/{h_nm:.2f}nm inconsistent with nominal {nominal_nm:.0f}nm via',desc=d,x_width_nm=w_nm,y_height_nm=h_nm)); continue
        ...
        final.append(d); rows.append({
            ...
            'selection_source':_source_of(d),'pixel_size_nm':px_nm,'via_nominal_diameter_nm':nominal_nm,'via_nominal_diameter_px':D,
    ...
    meta={
        ...
        'via_v24_nominal_diameter_nm':nominal_nm,'via_v24_nominal_diameter_px':D,'via_v24_grid_required':True,'via_v24_lattice_used':True,
    ...
    if pattern_key in ('via40', 'via60'):
        if level == 1:
            return {
                ...
                'V23_VIA_MIN_SPAN_FACTOR': 0.66,
                'V23_VIA_MAX_SPAN_FACTOR': 1.55,
        ...
        return {
            ...
            'V23_VIA_MIN_SPAN_FACTOR': 0.62,
            'V23_VIA_MAX_SPAN_FACTOR': 1.60,
    ...
    if pattern_key == 'trench160':
        if level == 1:
            return {
                ...
                'V20_TRENCH_GAP_FRAC_TOL': 0.42,
                'V20_TRENCH_WIDTH_FRAC_TOL': 0.68,
            }
        return {
            'V20_TRENCH_MIN_GROUPS': 2,
            'V20_TRENCH_MIN_PAIR_DARK_CONTRAST': 0.10,
            'V20_TRENCH_GAP_FRAC_TOL': 0.52,
            'V20_TRENCH_WIDTH_FRAC_TOL': 0.82,
        }
    return {}
...
# Yakun V7 primitives from the user-provided archive; see ALGORITHM_V31.md.
...
# V31: V27 localization + Yakun signal-supported edges + V30 missing-site recovery.
import copy as _copy_v31

_V31_MEASUREMENT_GRAY = None
_V31_PROCESS_BASE = _process_one_image_v19
_V31_SLOT_BASE = _select_and_measure_bottom_slots_v19
_V31_VIA_BASE = _select_and_measure_vias_v19
_V31_TRENCH_BASE = _select_and_measure_trenches_v19


def _v31_raw_gray(gray):
    return _V31_MEASUREMENT_GRAY if _V31_MEASUREMENT_GRAY is not None else gray


def _v31_read_measurement_gray(path):
...
def _process_one_image_v19(image_path, stage, condition, ann_dir):
    global _V31_MEASUREMENT_GRAY
    previous = _V31_MEASUREMENT_GRAY
    _V31_MEASUREMENT_GRAY = _v31_read_measurement_gray(image_path)
    try:
        return _V31_PROCESS_BASE(image_path, stage, condition, ann_dir)
    finally:
        _V31_MEASUREMENT_GRAY = previous
...
def _recover_local_slot_v18_deep(gray, expected_xy, x_pitch, row_pitch,
    ...
    if d is not None:
        return d
    d = _v31_recover_flatfield_slot(gray, expected_xy, x_pitch, row_pitch,
...
def _v31_contour_samples(contour):
...
def _v31_edge_source(gray, contour, extent):
...
def _v31_supported_contour(gray, contour):
    ...
    """
    samples = _v31_contour_samples(contour)
    if samples is None:
    ...
    source, roi_origin, background_slope = _v31_edge_source(gray, contour, extent)
    ...
    if not geometry_ok:
        ...
        if float(np.mean(anchor_supported)) >= .92 and _v31_max_cyclic_gap(anchor_supported) <= 3:
...
def _v31_refine_desc(gray, desc, pattern):
    contour, audit = _v31_supported_contour(_v31_raw_gray(gray), desc.contour)
    if contour is None:
        return None, audit
    ...
    if pattern == 'slot' and not _slot_shape_ok_v17(refined, (SLOT_MIN_AREA_PX, np.inf), seed=False):
        ...
        return None, audit
    refined._v31_edge_audit = audit
    return refined, audit


def _v31_edge_reject(desc, pattern, audit):
    return _make_reject_from_contour(desc.contour, f'{pattern}_v31_edge_qc',
                                    'original_image_edge_not_supported', str(audit.get('edge_status')),
                                    desc=desc, **audit)


def _v31_valid_slot_columns(gray, result, locator):
    ...
    for desc in selected:
        ...
        refined, audit = _v31_refine_desc(gray, desc, 'slot')
        if refined is None:
            ...
            if np.isfinite(xp) and np.isfinite(yp) and min(xp, yp) > 5:
                ...
                proposal = _v31_recover_flatfield_slot(_v31_raw_gray(gray), tuple(desc.center), xp, yp,
                ...
                if (proposal is not None and abs(proposal.center[0]-desc.center[0]) <= max(3., .25*xp)
                    ...
                    alternative, alternative_audit = _v31_refine_desc(gray, proposal, 'slot')
                    if alternative is not None:
                        refined, audit = alternative, alternative_audit
                        audit['edge_coarse_source'] = 'V30_repair_of_unsupported_V27_candidate'
        if refined is None:
            failed.add(column)
            rejects.append(_v31_edge_reject(desc, 'slot', audit))
        else:
    ...
    for column, members in columns.items():
        if column not in failed and set(members) == {1, 2, 3, 4} and column in templates:
            ...
            failed.add(column)
    meta = dict(meta, v31_edge_rejected_columns=len(failed))
...
def _select_and_measure_bottom_slots_v19(gray, pool, px_nm, image_name, stage, condition, pattern_key):
    print(f'      [{image_name}] V31: V27 slot localization + original-image edge validation', flush=True)
    primary = _V31_SLOT_BASE(gray, _copy_v31.deepcopy(pool), px_nm, image_name, stage, condition, pattern_key)
    columns, rejects, meta = _v31_valid_slot_columns(gray, primary, 'V27')
    ...
    if len(columns) < max(1, expected):
        extra = _v31_slot_flatfield_candidates(gray, pool)
        supplemented, added = merge_candidate_descs_v12(_copy_v31.deepcopy(pool), extra, 'slot')
        meta['v31_v30_locator_added'] = added
        if extra:
            print(f'      [{image_name}] V31: supplemental V30 proposals={len(extra)}; recheck original edges', flush=True)
            secondary = _V31_SLOT_BASE(gray, supplemented, px_nm, image_name, stage, condition, pattern_key)
            candidates, extra_rejects, secondary_meta = _v31_valid_slot_columns(gray, secondary, 'V30_recovery')
    ...
    for column_id, (members, template, locator) in enumerate(columns, 1):
        ...
        row = dict(template, object_id=column_id, slot_column_id_v23=column_id,
                   slot_complete_column_count=len(columns), v31_locator=locator,
                   v27_x_width_nm=template['x_width_nm'], v27_y_height_nm=template['y_height_nm'])
        row.update(desc._v31_edge_audit)
    ...
    meta.update(v23_complete_slot_columns=len(rows), slot_bottom_row_used_count=len(rows),
                ...
                v31_edge_method='V27 localization / Yakun original-gray normal tracking / V30 missing-site proposals')
...
def _select_and_measure_vias_v19(gray, pool, px_nm, image_name, stage, condition, pattern_key):
    rows, selected, rejects, bundle = _V31_VIA_BASE(gray, pool, px_nm, image_name, stage, condition, pattern_key)
    accepted, output = [], []
    for desc, old in zip(selected, rows):
        refined, audit = _v31_refine_desc(gray, desc, 'via')
        if refined is not None:
            ok, shape, _ = _via_shape_qc_v24(refined.contour, old['via_nominal_diameter_px'], stage, pattern_key)
            if not ok:
                refined = None
                audit['edge_status'] = 'REFINED_VIA_SHAPE_QC_FAILED'
        if refined is None:
            rejects.append(_v31_edge_reject(desc, 'via', audit))
            continue
        ...
        output.append(row)
    # Refit after rejecting a changed boundary, retaining V27's physical span gate.
    while accepted:
        ...
        for desc, row in zip(accepted, output):
            ...
            nominal = row['via_nominal_diameter_nm']
            if all(V23_VIA_MIN_SPAN_FACTOR*nominal <= span <= V23_VIA_MAX_SPAN_FACTOR*nominal for span in spans):
                kept.append((desc, row))
            else:
                audit = dict(desc._v31_edge_audit, edge_status='REFINED_VIA_PHYSICAL_SPAN_QC_FAILED')
                rejects.append(_v31_edge_reject(desc, 'via', audit))
    ...
    for index, (desc, row) in enumerate(zip(accepted, output)):
        ...
        row['x_width_nm'], row['y_height_nm'] = row['x_width_px']*px_nm, row['y_height_px']*px_nm
    bundle = dict(bundle, axis=axis, meta=dict(bundle['meta'], v31_edge_validated_count=len(output),
...
def _select_and_measure_trenches_v19(gray, pool, px_nm, image_name, stage, condition, pattern_key):
    rows, pairs, rejects, meta = _V31_TRENCH_BASE(gray, pool, px_nm, image_name, stage, condition, pattern_key)
    raw = _v31_raw_gray(gray).astype(float)
    if FEATURE_POLARITY.lower() == 'bright':
        raw = 255. - raw
    accepted, output = [], []
    for original, old in zip(pairs, rows):
        ...
        rejects.append(_make_reject_from_contour(c, 'trench_v31_edge_qc', 'original_image_edge_not_supported',
                                                 'Yakun width/tip signal checks failed or changed V27 geometry too far'))
    return output, accepted, rejects, dict(meta, v31_edge_validated_count=len(output))


# ============================================================================
# V31: MULTIFORMAT PIPELINE -- SLOT RECOVERY AND HARD PER-IMAGE TIMEOUT
# ============================================================================
...
import argparse as _argparse_v31
import json as _json_v31
import traceback as _traceback_v31
import pickle as _pickle_v31
import shutil as _shutil_v31
import subprocess as _subprocess_v31
import tempfile as _tempfile_v31
from typing import Any as _Any_v31, Iterable as _Iterable_v31, Sequence as _Sequence_v31

try:
    from scipy.optimize import linear_sum_assignment as _linear_sum_assignment_v31
except ImportError as _exc_v31:
    raise SystemExit(
        "V31 需要 scipy（逐对象 before/after 配对使用）。请运行：\n"
        "  pip install numpy pandas scipy opencv-python matplotlib openpyxl\n"
        f"原始错误：{_exc_v31}"
    ) from _exc_v31


V31_SCRIPT_VERSION = "2026-09-23-V31-V27-YAKUN-CONSERVATIVE-EDGE-FUSION"
V31_DEFAULT_BEFORE_ROOT = Path(r"C:\Users\z00027644\Documents\倾斜刻蚀\SEM\830SEM_before_treat")
V31_DEFAULT_AFTER_ROOT = Path(r"C:\Users\z00027644\Documents\倾斜刻蚀\SEM\0831SEM_10-14_topview_after")
V31_FIRST_REGION = 2
V31_PATTERN_SEQUENCE = ("trench", "slot", "via")
V31_STAGES = ("before", "after")
V31_STAGE_ZH = {"before": "处理前", "after": "处理后"}
V31_PATTERN_ZH = {"trench": "Trench", "slot": "Slot最下排", "via": "Via"}
V31_IMAGE_SUFFIXES = {".tif", ".tiff", ".png", ".jpg", ".jpeg"}
...
V31_FIXED_PATTERN_KEY = {
    "trench": "trench160",
    "slot": "slot210",
}
V31_VIA_KEYS = ("via40", "via60")
V31_VIA_NOMINAL_NM = {"via40": 40.0, "via60": 60.0}
...
V31_MATCH_THRESHOLD_VIA = 0.30
V31_MATCH_THRESHOLD_1D = 0.28


# -----------------------------------------------------------------------------
# V31 generic helpers
# -----------------------------------------------------------------------------
def _v31_condition_key(condition: str) -> tuple:
    name = str(condition)
    return int(name), name


def _v31_discover_conditions(after_root: Path) -> List[str]:
...
def _v31_sort_conditions(df: pd.DataFrame, columns: _Sequence_v31[str]) -> pd.DataFrame:
    """Sort condition names numerically while retaining exact names, e.g. 007."""
    if df.empty:
        return df
    out = df.copy()
    names = sorted(out["condition"].astype(str).unique(), key=_v31_condition_key)
    order = {name: index for index, name in enumerate(names)}
    out["_condition_order_v31"] = out["condition"].astype(str).map(order)
    sort_columns = ["_condition_order_v31"] + [c for c in columns if c != "condition"]
    return out.sort_values(sort_columns, kind="stable").drop(columns=["_condition_order_v31"]).reset_index(drop=True)


def _v31_finite_float(value: _Any_v31, default: float = math.nan) -> float:
    try:
        x = float(value)
    except Exception:
        return default
...
def _v31_safe_mean(values: _Iterable_v31) -> float:
...
def _v31_safe_median(values: _Iterable_v31) -> float:
    a = pd.to_numeric(pd.Series(list(values), dtype="object"), errors="coerce").dropna().to_numpy(float)
    return float(np.median(a)) if len(a) else math.nan


def _v31_natural_key(path: Path) -> tuple:
    parts = re.split(r"(\d+)", path.stem.lower())
    return (tuple(int(p) if p.isdigit() else p for p in parts), path.name.lower(), path.name)


def _v31_df(rows) -> pd.DataFrame:
    return pd.DataFrame(rows) if rows else pd.DataFrame()


def _v31_write_csv(df: pd.DataFrame, path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    df.to_csv(path, index=False, encoding="utf-8-sig")


def _v31_expected_positions(image_count: int) -> List[dict]:
    """Extend the original trench/slot/via order to every discovered image."""
    return [
        {
            "sequence_index": index + 1,
            "region": V31_FIRST_REGION + index // len(V31_PATTERN_SEQUENCE),
            "pattern": V31_PATTERN_SEQUENCE[index % len(V31_PATTERN_SEQUENCE)],
        }
...
# -----------------------------------------------------------------------------
def _v31_discover_one_condition(stage: str, root: Path, condition: str):
    ...
    images = sorted(
        [p for p in folder.iterdir() if p.is_file() and p.suffix.lower() in V31_IMAGE_SUFFIXES],
        key=_v31_natural_key,
    )

    for pos, image_path in zip(_v31_expected_positions(len(images)), images):
        ...
        rows.append({
            "stage": stage,
            "stage_zh": V31_STAGE_ZH[stage],
            "condition": str(condition),
            "condition_label": f"Condition {condition}",
            "region": int(pos["region"]),
            "pattern": str(pos["pattern"]),
            "pattern_zh": V31_PATTERN_ZH[str(pos["pattern"])],
...
def _v31_build_inventory(
    before_root: Path, after_root: Path, conditions: Optional[_Sequence_v31[str]] = None,
):
    if conditions is None:
        conditions = _v31_discover_conditions(after_root)
    rows: List[dict] = []
    errors: List[dict] = []
    for condition in conditions:
        condition = str(condition)
        before_rows, before_errors = _v31_discover_one_condition("before", before_root, condition)
        after_rows, after_errors = _v31_discover_one_condition("after", after_root, condition)
    ...
    if not inv.empty:
        inv = _v31_sort_conditions(inv, ["condition", "region", "sequence_index", "stage"])
    return inv, err
...
def _v31_process_v25_assigned(
    ...
    """Assign pattern keys to the embedded pipeline, including V31 slot recovery."""
    ...
    for r0 in rows:
        ...
        r["pattern_zh"] = V31_PATTERN_ZH[public_pattern]
        r["region"] = int(region)
        r["pair_key"] = pair_key
        r["source_algorithm"] = "V31_V27_Yakun_fusion"
        r["x_width_nm"] = _v31_finite_float(r.get("x_width_nm"))
        r["y_height_nm"] = _v31_finite_float(r.get("y_height_nm"))
        r["tip_gap_y_nm"] = _v31_finite_float(r.get("tip_gap_y_nm"))
        normalized_rows.append(r)

    status = dict(status)
    status["source_pattern_key"] = status.get("pattern", v25_pattern_key)
    status["pattern"] = public_pattern
    status["pattern_zh"] = V31_PATTERN_ZH[public_pattern]
    status["region"] = int(region)
    status["pair_key"] = pair_key
    status["source_algorithm"] = "V31_V27_Yakun_fusion"
    ...
    for q0 in rejects:
        ...
        q["region"] = int(region)
        q["pair_key"] = pair_key
        q["source_algorithm"] = "V31_V27_Yakun_fusion"
...
# -----------------------------------------------------------------------------
def _v31_via_trial_quality(rows: List[dict], status: dict, key: str) -> tuple:
    ...
    n = int(len(df))
    fallback_level = int(_v31_finite_float(status.get("auto_fallback_level_used", 0), 0.0))
    strict = 1 if fallback_level == 0 else 0
    occupancy = _v31_finite_float(status.get("measure_array_occupancy_seed"), -1.0)
    good_ray = _v31_safe_mean(df.get("via_good_ray_fraction", pd.Series(dtype=float)).tolist())
    scale_score = _v31_safe_mean(df.get("via_scale_match_score", pd.Series(dtype=float)).tolist())
    leak = _v31_safe_mean(df.get("via_outer_dark_leak_fraction", pd.Series(dtype=float)).tolist())
    ...
    nominal = V31_VIA_NOMINAL_NM[key]
...
def _v31_choose_via_key_from_before(
    before_path: Path,
    condition: str,
    region: int,
    trial_root: Path,
    ...
    for key in V31_VIA_KEYS:
        ann_dir = trial_root / str(condition) / f"Region_{region}" / key
        try:
            rows, status, _ = _v31_process_v25_assigned(
                before_path, "before", str(condition), int(region), "via", key, ann_dir
            )
            quality = _v31_via_trial_quality(rows, status, key)
            err = ""
            n = len(rows)
            fallback = int(_v31_finite_float(status.get("auto_fallback_level_used", 0), 0.0))
            medx = _v31_safe_median(pd.DataFrame(rows).get("x_width_nm", pd.Series(dtype=float)).tolist()) if rows else math.nan
            medy = _v31_safe_median(pd.DataFrame(rows).get("y_height_nm", pd.Series(dtype=float)).tolist()) if rows else math.nan
        except Exception as exc:
        ...
        trial_rows.append({
            ...
            "trial_nominal_nm": V31_VIA_NOMINAL_NM[key],
            "mode": "AUTO_V25_ONLY",
            "selected": False,
            "n_measurements": n,
...
# -----------------------------------------------------------------------------
def _v31_build_image_metrics(objects_df: pd.DataFrame) -> pd.DataFrame:
    if objects_df.empty:
        return pd.DataFrame()
    records: List[dict] = []
    group_cols = [
    ...
    for keys, g in objects_df.groupby(group_cols, dropna=False, sort=True):
        ...
        for source_col, metric, metric_zh in V31_METRIC_SPECS[pattern]:
    ...
    return _v31_sort_conditions(pd.DataFrame(records), ["condition", "region", "pattern", "stage", "image", "metric"])


def _v31_build_image_pair_comparison(image_metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    porder = {p: i for i, p in enumerate(V31_PATTERN_SEQUENCE)}
    out["_po"] = out["pattern"].map(porder).fillna(999)
    out = _v31_sort_conditions(out, ["condition", "region", "_po", "metric"]).drop(columns=["_po"])
    return out


def _v31_build_overall_comparison(objects_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for (stage, condition, pattern), g in objects_df.groupby(["stage", "condition", "pattern"], sort=True):
        for source_col, metric, metric_zh in V31_METRIC_SPECS[str(pattern)]:
            vals = pd.to_numeric(g.get(source_col, pd.Series(dtype=float)), errors="coerce").dropna().to_numpy(float)
            if not len(vals):
                continue
            long_rows.append({
                ...
                "condition_label": f"Condition {condition}",
                "pattern": pattern,
                "pattern_zh": V31_PATTERN_ZH[str(pattern)],
                "metric": metric,
                "metric_zh": metric_zh,
                "source_column": source_col,
                "n": int(len(vals)),
    ...
    return _v31_sort_conditions(out, ["condition", "pattern", "metric"])


# -----------------------------------------------------------------------------
# Per-object spatial before/after matching (comparison only)
# -----------------------------------------------------------------------------
def _v31_normalized_1d(g: pd.DataFrame) -> np.ndarray:
...
def _v31_normalized_2d(g: pd.DataFrame) -> np.ndarray:
...
def _v31_match_objects(before: pd.DataFrame, after: pd.DataFrame, pattern: str):
    ...
    if pattern == "via":
        pb, pa = _v31_normalized_2d(b), _v31_normalized_2d(a)
        threshold = V31_MATCH_THRESHOLD_VIA
    else:
        pb = _v31_normalized_1d(b)[:, None]
        pa = _v31_normalized_1d(a)[:, None]
        threshold = V31_MATCH_THRESHOLD_1D

    cost = np.linalg.norm(pb[:, None, :] - pa[None, :, :], axis=2)
    bi, aj = _linear_sum_assignment_v31(cost)
    accepted = {}
    used_a = set()
    dist = {}
...
def _v31_build_object_comparison(objects_df: pd.DataFrame) -> pd.DataFrame:
    if objects_df.empty:
        return pd.DataFrame()
    records: List[dict] = []
    ...
    for _, k in keys.iterrows():
        ...
        pairs = _v31_match_objects(before, after, pattern)
        for pair_id, (b, a, match_distance) in enumerate(pairs, 1):
            base = {
                ...
                "pair_key": pair_key,
                "pattern": pattern,
                "pattern_zh": V31_PATTERN_ZH[pattern],
            ...
            for source_col, metric, metric_zh in V31_METRIC_SPECS[pattern]:
                bv = _v31_finite_float(b.get(source_col)) if b is not None else math.nan
                av = _v31_finite_float(a.get(source_col)) if a is not None else math.nan
                ...
                records.append(rec)
    return _v31_sort_conditions(pd.DataFrame(records), ["condition", "region", "pattern", "matched_pair_id", "metric"])


# -----------------------------------------------------------------------------
# Plotting
# -----------------------------------------------------------------------------
def _v31_safe_plot_name(s: str) -> str:
    return re.sub(r"[^A-Za-z0-9_.-]+", "_", str(s))


def _v31_save_pair_plots(pair_df: pd.DataFrame, plot_dir: Path) -> None:
    if pair_df.empty:
        return
    plot_dir.mkdir(parents=True, exist_ok=True)
    for (condition, pattern, metric), g in pair_df.groupby(["condition", "pattern", "metric"], sort=True):
        ...
        ax.grid(axis="y", alpha=0.25)
        fig.tight_layout()
        out = plot_dir / f"C{condition}__{pattern}__{_v31_safe_plot_name(metric)}__before_after.png"
    ...
    for (pattern, metric), g in pair_df.groupby(["pattern", "metric"], sort=True):
        ...
        g = _v31_sort_conditions(g, ["condition", "region"])
        ...
        ax.grid(axis="y", alpha=0.25)
        fig.tight_layout()
        out = plot_dir / f"ALL__{pattern}__{_v31_safe_plot_name(metric)}__delta.png"
        fig.savefig(out, dpi=220)
        plt.close(fig)
...
# Main V31 pipeline
# -----------------------------------------------------------------------------
class V31ImageTimeout(TimeoutError):
    """The image's complete processing budget, including via trials, expired."""


def _v31_measure_image_job(job: dict):
    ...
    if pattern == 'via' and stage == 'before' and not key:
        best = None
        for candidate_key in V31_VIA_KEYS:
            try:
                payload = _v31_process_v25_assigned(
                    image_path, stage, job['condition'], job['region'], pattern,
                    candidate_key, ann_dir / '_via_trials' / candidate_key)
                quality = _v31_via_trial_quality(payload[0], payload[1], candidate_key)
                trials.append(dict(trial_pattern_key=candidate_key, n_measurements=len(payload[0]),
                                   quality_key=repr(quality), error='',
                                   fallback_level=int(_v31_finite_float(payload[1].get('auto_fallback_level_used', 0), 0)),
                                   median_x_width_nm=_v31_safe_median([r.get('x_width_nm', np.nan) for r in payload[0]]),
                                   median_y_height_nm=_v31_safe_median([r.get('y_height_nm', np.nan) for r in payload[0]])))
        ...
        if annotation is not None and annotation.is_file():
            ...
            _shutil_v31.copy2(annotation, selected_path)
        ...
        rows, status, rejects = _v31_process_v25_assigned(
    ...
    for trial in trials:
        trial.update(condition=job['condition'], region=job['region'], before_image=image_path.name,
                     ...
                     trial_nominal_nm=V31_VIA_NOMINAL_NM[trial['trial_pattern_key']],
...
def _v31_worker_main(request_path: str) -> int:
    request = _json_v31.loads(Path(request_path).read_text(encoding='utf-8'))
    try:
        result = {'ok': True, 'payload': _v31_measure_image_job(request)}
    except Exception as exc:
        result = {'ok': False, 'error': f'{type(exc).__name__}: {exc}',
                  'traceback': _traceback_v31.format_exc()}
    with Path(request['result_path']).open('wb') as stream:
        _pickle_v31.dump(result, stream, protocol=_pickle_v31.HIGHEST_PROTOCOL)
    return 0


def _v31_process_image_with_timeout(
    ...
    with _tempfile_v31.TemporaryDirectory(prefix='sem_v31_image_') as temporary:
        root = Path(temporary)
        ...
        request_path.write_text(_json_v31.dumps(request, ensure_ascii=False), encoding='utf-8')
        process = _subprocess_v31.Popen([sys.executable, '-u', str(Path(__file__).resolve()),
                                        '--_v31-worker', str(request_path)])
        last_progress = started
        try:
            while True:
                remaining = deadline - time.monotonic()
                if remaining <= 0:
                    raise V31ImageTimeout(f'{Path(image_path).name}: exceeded {timeout_seconds:g}s; image skipped')
                try:
                    process.wait(timeout=min(1.0, remaining))
                    break
                except _subprocess_v31.TimeoutExpired:
                    now = time.monotonic()
                    if now - last_progress >= 15:
                        print(f'    V31 {Path(image_path).name}: elapsed={now-started:.0f}s / {timeout_seconds:g}s', flush=True)
                        last_progress = now
            ...
            with result_path.open('rb') as stream:
                result = _pickle_v31.load(stream)
            if not result['ok']:
            ...
            for source in scratch_annotations.rglob('*'):
                if source.is_file():
                    ...
                    _shutil_v31.copy2(source, destination)
...
def main_v31(
    ...
    conditions = _v31_discover_conditions(after_root)
    if output_root is None:
        output_root = after_root.parent / "SEM_0830_0831_compare_V31"
    output_root = Path(output_root)
    output_root.mkdir(parents=True, exist_ok=True)
    ...
    print("=" * 100)
    print("V31 | SEM comparison | V27 geometry + Yakun edge evidence + per-image timeout")
    ...
    inventory_df, preflight_errors_df = _v31_build_inventory(before_root, after_root, conditions)
    _v31_write_csv(inventory_df, output_root / "inventory_mapping.csv")
    _v31_write_csv(preflight_errors_df, output_root / "processing_errors.csv")
    ...
    _pattern_order_v31 = {p: i for i, p in enumerate(V31_PATTERN_SEQUENCE)}
    pair_keys["_pattern_order_v31"] = pair_keys["pattern"].map(_pattern_order_v31).fillna(999)
    pair_keys = _v31_sort_conditions(pair_keys, ["condition", "region", "_pattern_order_v31"]).drop(columns=["_pattern_order_v31"])
    ...
    for _, pk in pair_keys.iterrows():
        ...
        print(f"\n[{pair_key}] {V31_PATTERN_ZH[pattern]}")
        stage_paths = {}
        valid = True
        for stage in V31_STAGES:
        ...
        if pattern in V31_FIXED_PATTERN_KEY:
            selected_key = V31_FIXED_PATTERN_KEY[pattern]
        else:
            selected_key = via_pattern_mode if via_pattern_mode in V31_VIA_KEYS else None
        ...
        for stage in V31_STAGES:
            ...
            if pattern == 'via' and stage == 'after' and selected_key is None:
                ...
                continue
            print(f"  {V31_STAGE_ZH[stage]} {image_path.name} -> {selected_key}")
            try:
                rows, status, rejects, used_key, trials = _v31_process_image_with_timeout(
                ...
                processing_errors.append({
                    ...
                    "error_type": 'IMAGE_TIMEOUT' if isinstance(exc, V31ImageTimeout) else type(exc).__name__,
                    "message": str(exc),
                    "traceback": _traceback_v31.format_exc(),
                })
                status_rows.append({
                    ...
                    "path": str(image_path),
                    "selected_v25_pattern_key": selected_key,
                    "source_algorithm": "V31_V27_Yakun_fusion",
                    "status": "SKIPPED_TIMEOUT" if isinstance(exc, V31ImageTimeout) else "ERROR",
                    "image_timeout_seconds": image_timeout_seconds,
                    "note": f"{type(exc).__name__}: {exc}",
                })
                label = 'TIMEOUT -> SKIP' if isinstance(exc, V31ImageTimeout) else 'ERROR'
                print(f"    {label} {type(exc).__name__}: {exc}", flush=True)
            # Keep completed statuses and skip reasons available during long runs.
            _v31_write_csv(_v31_df(status_rows), output_root / 'image_status.csv')
            _v31_write_csv(_v31_df(processing_errors), output_root / 'processing_errors.csv')

    objects_df = _v31_df(all_rows)
    status_df = _v31_df(status_rows)
    rejected_df = _v31_df(rejected_rows)
    errors_df = _v31_df(processing_errors)
    via_choice_df = _v31_df(via_choice_rows)

    if not objects_df.empty:
        sort_cols = [c for c in ["condition", "region", "pattern", "stage", "image", "object_id"] if c in objects_df.columns]
        objects_df = _v31_sort_conditions(objects_df, sort_cols)

    image_metrics_df = _v31_build_image_metrics(objects_df)
    image_pair_df = _v31_build_image_pair_comparison(image_metrics_df)
    overall_df = _v31_build_overall_comparison(objects_df)
    object_pair_df = _v31_build_object_comparison(objects_df)

    outputs = {
        "excel": output_root / "SEM_0830_0831_before_after_V31_results.xlsx",
        ...
        "via_model_choice": output_root / "via_v25_model_choice.csv",
        "errors": output_root / "processing_errors.csv",
        "settings": output_root / "settings.json",
    }

    _v31_write_csv(inventory_df, outputs["inventory"])
    _v31_write_csv(status_df, outputs["image_status"])
    _v31_write_csv(objects_df, outputs["all_objects"])
    _v31_write_csv(image_metrics_df, outputs["image_metrics"])
    _v31_write_csv(image_pair_df, outputs["image_pair_comparison"])
    _v31_write_csv(overall_df, outputs["overall_comparison"])
    _v31_write_csv(object_pair_df, outputs["object_comparison"])
    _v31_write_csv(rejected_df, outputs["rejected_objects"])
    _v31_write_csv(via_choice_df, outputs["via_model_choice"])
    _v31_write_csv(errors_df, outputs["errors"])

    settings = {
        "script_version": V31_SCRIPT_VERSION,
        "algorithm_base": "V27 geometry/output + Yakun gray edge primitives + V30 bounded recovery/timeout",
        "v31_edge_rule": "original-image local gray 50%; contour normals with circular tracking; no forced ellipse/rectangle",
        "v31_edge_min_support_fraction": 0.85,
        "v31_edge_max_missing_fraction": 0.08,
        "v31_illumination_rule": "robust exterior plane subtraction for contour profiles; enhanced images are localization only",
        "v31_slot_recovery_rule": "V27 first; V30 only missing columns or contours failing original-gray evidence; four supported rows required",
        "image_timeout_seconds": image_timeout_seconds,
        ...
        "pattern_sequence_per_region": list(V31_PATTERN_SEQUENCE),
        "supported_image_extensions": sorted(V31_IMAGE_SUFFIXES),
        ...
        "via_auto_rule": "test via40 and via60 using V31 on BEFORE only; select by validated result quality; freeze same key for AFTER",
        "metric_specs": V31_METRIC_SPECS,
    ...
    outputs["settings"].write_text(_json_v31.dumps(settings, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    _v31_save_pair_plots(image_pair_df, plot_dir)
...
# V31 CLI
# -----------------------------------------------------------------------------
def _v31_build_parser():
    parser = _argparse_v31.ArgumentParser(
        description=(
            "V31: SEM comparison using numeric-named folders discovered under AFTER, "
            "matched by exact folder name in BEFORE, with equal image counts (TIF/TIFF/PNG/JPG/JPEG). "
            "V31 combines V27 geometry, Yakun edges and V30 bounded recovery; 300s image limit."
        )
    )
    parser.add_argument(
        "--before", type=Path, default=V31_DEFAULT_BEFORE_ROOT,
        help=f"处理前根目录；默认：{V31_DEFAULT_BEFORE_ROOT}",
    )
    parser.add_argument(
        "--after", type=Path, default=V31_DEFAULT_AFTER_ROOT,
        help=f"处理后根目录；自动识别其纯数字名称子文件夹；默认：{V31_DEFAULT_AFTER_ROOT}",
    )
    parser.add_argument(
        "--output", type=Path, default=None,
        help="输出目录；默认在两个输入目录同级建立 SEM_0830_0831_compare_V31",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "Via 模型。auto=在每个 BEFORE via 图上用 V31 分别试 via40/via60，"
            "选更可靠者并固定给对应AFTER；若已知图案类型可强制via40或via60。"
        ),
    )
    return parser


def _v31_cli(argv=None) -> int:
    args = _v31_build_parser().parse_args(argv)
    try:
        before_root = Path(args.before)
        after_root = Path(args.after)
        output_root = Path(args.output) if args.output is not None else after_root.parent / "SEM_0830_0831_compare_V31"
        main_v31(
        ...
        print(f"V31 程序失败：{type(exc).__name__}: {exc}", file=sys.stderr)
        _traceback_v31.print_exc()
        return 1


if __name__ == "__main__":
    if len(sys.argv) == 3 and sys.argv[1] == '--_v31-worker':
        raise SystemExit(_v31_worker_main(sys.argv[2]))
    raise SystemExit(_v31_cli())
