...
def _fit_four_row_centers_v17(
    candidates: List[ShapeDesc], image_h: int
) -> Optional[Dict[str, object]]:
    """
    ...
    best = None
    for p in hypotheses:
        tol = max(5.0, V17_SLOT_ROW_MODEL_RESIDUAL_FRAC * p)
        for y in ys:
            for k0 in range(4):
                y0 = float(y - k0 * p)
                centers = y0 + p * np.arange(4, dtype=float)
                # Permit the first/last row center near the image edge, but reject
                # models that place most of the nominal four rows far outside frame.
                inside = np.sum((centers >= -0.10 * image_h) & (centers <= 1.10 * image_h))
                if inside < 3:
                    continue
                dist = np.abs(ys[:, None] - centers[None, :])
                ridx = np.argmin(dist, axis=1)
                res = dist[np.arange(len(ys)), ridx]
                inl = res <= tol
                n_in = int(np.sum(inl))
                if n_in < 2:
                    continue
                occ = len(set(int(v) for v in ridx[inl]))
                med_res = float(np.median(res[inl])) if n_in else 1e9
                # Favor coverage of distinct rows, then total candidate support.
                score = 5.0 * occ + 1.0 * n_in - 1.6 * med_res / max(p, 1e-9)
                rec = (score, occ, n_in, -med_res, y0, p, ridx, inl)
...
def _slot_lowgray_model_proposals_v18(gray: np.ndarray) -> List[ShapeDesc]:
...
def _slot_four_row_array_v18(
    ...
    if row_model is None or len(model_cands)<8:
        ...
        row_model=_fit_four_row_centers_v17(_dedup_desc_list_v16(model_cands+lowgray_loc),H)
    meta={
...
def _annotate_slots_v19(gray: np.ndarray, selected_all: List[ShapeDesc], bottom: List[ShapeDesc], rows: List[dict]) -> np.ndarray:
    ...
    for d in bottom:
        ...
        cv2.drawContours(out,[d.contour.astype(np.int32)],-1,(0,220,0),2); c=tuple(np.round(d.center).astype(int))
        if r is not None:
...
def _v27_build_inventory(before_root: Path, after_root: Path, conditions: _Sequence_v27[str]):
    ...
    if not inv.empty:
        inv = inv.sort_values(["condition", "region", "sequence_index", "stage"]).reset_index(drop=True)
    return inv, err
...
def _v27_build_overall_comparison(objects_df: pd.DataFrame) -> pd.DataFrame:
    ...
    return out.sort_values(["condition", "pattern", "metric"]).reset_index(drop=True)
...
def _v27_save_pair_plots(pair_df: pd.DataFrame, plot_dir: Path) -> None:
    ...
    for (pattern, metric), g in pair_df.groupby(["pattern", "metric"], sort=True):
        ...
        g = g.sort_values(["condition", "region"])
...
def main_v27(
    ...
    # V25 uses these globals to write labels. Expand them to 10-14.
...
def _v27_build_parser():
    ...
    parser.add_argument(
        ...
        help=(
            "V25 via模型。auto=在每个 BEFORE via 图上只用V25分别试via40/via60，"
