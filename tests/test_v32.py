"""V32 signal accuracy, shape preservation, original-gray gates and hard deadlines."""
import contextlib
import io
import json
import sys
...
sys.path.insert(0, str(ROOT / 'SEM_0830_0831_compare_V32_bundle'))
import sem_before_after_compare_0831_V32 as sem
...
class EdgeAccuracyTests(unittest.TestCase):
    def test_measurement_reader_preserves_8bit_and_unclipped_16bit_signal(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            np.testing.assert_array_equal(sem._v32_read_measurement_gray(path), original)
            high_depth = original.astype(np.uint16)*173
            sem._imwrite_unicode_v19(path, high_depth)
            decoded = sem._v32_read_measurement_gray(path)
        ...
        for power in (2, 6):
            for angle in (0., 23.):
                with self.subTest(power=power, angle=angle):
                    ...
                    contour, audit = sem._v32_supported_contour(gray, coarse)
    ...
    def test_enhanced_locator_cannot_replace_original_measurement_pixels(self):
        ...
        with patch.object(sem, '_V32_MEASUREMENT_GRAY', original):
            contour, _ = sem._v32_refine_desc(enhanced, desc, 'via')
        self.assertIsNone(contour)

    def test_original_gray_context_restored_even_after_failure(self):
        old = sem._V32_MEASUREMENT_GRAY
        image = np.ones((5, 5), np.uint8)
        def fail(*args):
            self.assertIs(sem._V32_MEASUREMENT_GRAY, image)
            raise RuntimeError('measurement failure')
        with patch.object(sem, '_v32_read_measurement_gray', return_value=image), \
             patch.object(sem, '_V32_PROCESS_BASE', side_effect=fail):
            with self.assertRaises(RuntimeError):
                sem._process_one_image_v19(Path('image.png'), 'before', '1', Path('ann'))
        self.assertIs(sem._V32_MEASUREMENT_GRAY, old)
...
class FusionPipelineTests(unittest.TestCase):
    def test_file_to_measurement_uses_raw_gray_with_v27_locator(self):
        with tempfile.TemporaryDirectory() as temporary, contextlib.redirect_stdout(io.StringIO()):
            ...
            rows, status, _ = sem._v32_process_v25_assigned(image, 'before', '007', 2, 'slot', 'slot210', root/'ann')
    ...
    def test_valid_v27_columns_are_not_replaced_by_v30(self):
        ...
        pool = sem._v32_slot_flatfield_candidates(gray, [])
        with contextlib.redirect_stdout(io.StringIO()), \
             patch.object(sem, '_v32_slot_flatfield_candidates', side_effect=AssertionError('unnecessary V30 replacement')):
            rows, _, _, _, _ = sem._select_and_measure_bottom_slots_v19(gray, pool, .8, 'good.png', 'before', '1', 'slot210')
        self.assertEqual(len(rows), 3)
        self.assertEqual({r['v32_locator'] for r in rows}, {'V27'})

    def test_missing_bottom_does_not_produce_a_complete_column(self):
        gray = gradient_slots(40.)
        pool = sem._v32_slot_flatfield_candidates(gray, [])
        with contextlib.redirect_stdout(io.StringIO()):
            original = sem._V32_SLOT_BASE(gray, pool, .8, 'missing.png', 'before', '1', 'slot210')
        yy, xx = np.mgrid[:400, :360]
        background = (80.+.24*xx+.12*yy).astype(np.uint8)
        gray[305:345, 160:200] = background[305:345, 160:200]
        columns, rejects, _ = sem._v32_valid_slot_columns(gray, original, 'V27')
...
class TimeoutTests(unittest.TestCase):
    ...
    def test_hard_timeout_kills_worker_and_discards_partial_output(self):
        ...
        processes = []
        original_popen = sem._subprocess_v32.Popen
        ...
        with patch.object(sem, '__file__', str(fake_worker)), \
                patch.object(sem._subprocess_v32, 'Popen', side_effect=start):
            with self.assertRaises(sem.V32ImageTimeout):
                sem._v32_process_image_with_timeout(
    ...
    def test_real_standalone_worker_publishes_valid_result(self):
        rows, status, _, key, trials = sem._v32_process_image_with_timeout(
            self.image(), 'before', '007', 2, 'slot', 'slot210',
            self.root / 'annotations', timeout_seconds=30.)
        self.assertEqual(len(rows), 3)
        self.assertEqual(key, 'slot210')
        self.assertEqual(trials, [])
        self.assertEqual(status['source_algorithm'], 'V32_V27_Yakun_fusion')
        self.assertTrue(Path(status['annotated_path']).is_file())
    ...
    def test_both_via_trials_share_job_and_winner_is_not_reprocessed(self):
        ...
        with patch.object(sem, '_v32_process_v25_assigned', side_effect=measure) as process:
            _, _, _, key, trials = sem._v32_measure_image_job(job)
    ...
    def test_batch_exports_timeout_and_continues_later_images(self):
        ...
        def raw(image_path, stage, condition, ann_dir):
            row = dict(stage=stage, stage_zh=sem.V32_STAGE_ZH[stage], condition=condition,
        ...
        def process(**kw):
            ...
            if len(calls) == 1:
                raise sem.V32ImageTimeout('test image exceeded 300s')
            args = {k: v for k, v in kw.items() if k != 'timeout_seconds'}
            rows, status, rejects = sem._v32_process_v25_assigned(**args)
            return rows, status, rejects, kw['v25_pattern_key'], []
        output = self.root / 'output'
        with patch.object(sem, '_process_one_image_v19', side_effect=raw), \
                patch.object(sem, '_v32_process_image_with_timeout', side_effect=process), \
                patch.object(sem, '_v32_save_pair_plots'), contextlib.redirect_stdout(io.StringIO()):
            paths = sem.main_v32(self.root / 'before', self.root / 'after', output)
        ...
        self.assertIn('V32', settings['script_version'])
        self.assertEqual(paths['excel'].name, 'SEM_0830_0831_before_after_V32_results.xlsx')
    ...
    def test_auto_via_timeout_skips_dependent_after_and_continues_next_region(self):
        ...
        def process(**kw):
            ...
            if kw['public_pattern'] == 'via':
                ...
                raise sem.V32ImageTimeout('via trials exhausted the shared image deadline')
        ...
        with patch.object(sem, '_v32_process_image_with_timeout', side_effect=process), \
                patch.object(sem, '_v32_save_pair_plots'), contextlib.redirect_stdout(io.StringIO()):
            paths = sem.main_v32(self.root / 'before', self.root / 'after', self.root / 'output', via_pattern_mode='auto')
...
class CDPolicyTests(unittest.TestCase):
    def test_default_model_is_via60_for_cli_and_batch_both_stages(self):
        self.assertEqual(sem._v32_build_parser().parse_args([]).via_pattern, 'via60')
        for key in ('via40', 'via60', 'auto'):
            self.assertEqual(sem._v32_build_parser().parse_args(['--via-pattern', key]).via_pattern, key)
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            for stage in ('before', 'after'):
                folder = root / stage / '007'
                folder.mkdir(parents=True)
                for index in range(1, 4):
                    (folder / f'{index}.png').touch()
                    (folder / f'{index}.txt').write_text('PixelSize=2.5')
            calls = []
            def process(**kw):
                calls.append((kw['public_pattern'], kw['stage'], kw['v25_pattern_key']))
                status = dict(stage=kw['stage'], condition='007', region=kw['region'],
                              pattern=kw['public_pattern'], image=kw['image_path'].name, status='OK')
                return [], status, [], kw['v25_pattern_key'], []
            with patch.object(sem, '_v32_process_image_with_timeout', side_effect=process), \
                    patch.object(sem, '_v32_save_pair_plots'), contextlib.redirect_stdout(io.StringIO()):
                outputs = sem.main_v32(root/'before', root/'after', root/'output')
            self.assertEqual([c for c in calls if c[0] == 'via'],
                             [('via', 'before', 'via60'), ('via', 'after', 'via60')])
            settings = json.loads(outputs['settings'].read_text())
            self.assertEqual(settings['via_pattern_mode'], 'via60')
            self.assertIsNone(settings['cd_filter_policy']['slot_trench_fixed_nm_limits'])
            status = sem.pd.read_csv(outputs['image_status'])
            self.assertEqual(status.loc[status['pattern']=='via', 'selected_pattern_key'].tolist(), ['via60', 'via60'])

    def test_via60_above_old_cd_ceilings_keeps_actual_boundaries(self):
        # The exact same 40-pixel boundaries represent different physical sizes.
        # A nominal-nm gate must not change the selected contours with calibration.
        for cd_nm, stage in ((65., 'before'), (100., 'after'), (120., 'before'), (240., 'after')):
            with self.subTest(cd_nm=cd_nm, stage=stage):
                gray, pool = analytic_vias()
                px_nm = cd_nm / 40.
                with contextlib.redirect_stdout(io.StringIO()):
                    rows, selected, _, bundle = sem._select_and_measure_vias_v19(
                        gray, pool, px_nm, 'via.png', stage, '1', 'via60')
                self.assertEqual(len(rows), 20)
                np.testing.assert_allclose([r['x_width_px'] for r in rows], 40., atol=.7)
                np.testing.assert_allclose([r['y_height_px'] for r in rows], 40., atol=.7)
                for row in rows:
                    self.assertEqual(row['pattern'], 'via60')
                    self.assertEqual(row['via_nominal_diameter_nm'], 60.)
                    self.assertAlmostEqual(row['x_width_nm'], row['x_width_px']*px_nm)
                    self.assertEqual(row['via_search_scale_source'], 'candidate_pool_median')
                    self.assertGreaterEqual(row['edge_support_fraction'], .85)
                self.assertAlmostEqual(bundle['meta']['via_search_diameter_nm'], cd_nm, delta=.05*cd_nm)

    def test_wider_pixel_vias_are_not_limited_to_old_search_radius(self):
        yy, xx = np.mgrid[:440, :540]
        radius = np.full(xx.shape, 1000.)
        for y in (70, 170, 270, 370):
            for x in (70, 170, 270, 370, 470):
                radius = np.minimum(radius, np.hypot(xx-x, yy-y))
        gray = (185. - 120./(1.+np.exp(np.clip((radius-35.)/.8, -100, 100)))).astype(np.uint8)
        primary, _ = sem.extract_descriptors_with_rejections(sem.segment_features(gray))
        pool, _ = sem.build_candidate_pool_v12(gray, primary, 'via', 'wide.png')
        with contextlib.redirect_stdout(io.StringIO()):
            rows, _, _, _ = sem._select_and_measure_vias_v19(gray, pool, 2., 'wide.png', 'before', '1', 'via60')
        self.assertEqual(len(rows), 20)
        np.testing.assert_allclose([r['x_width_nm'] for r in rows], 140., atol=1.4)
        np.testing.assert_allclose([r['y_height_nm'] for r in rows], 140., atol=1.4)

    def test_real_worker_uses_via60_and_exports_scale_above_90nm(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            path = root / 'via.png'
            gray, _ = analytic_vias()
            sem._imwrite_unicode_v19(path, gray)
            path.with_suffix('.txt').write_text('PixelSize=3.0')
            rows, status, _, key, trials = sem._v32_process_image_with_timeout(
                path, 'before', '007', 2, 'via', 'via60', root/'annotated', timeout_seconds=60.)
            self.assertEqual(key, 'via60')
            self.assertEqual(len(rows), 20)
            np.testing.assert_allclose([r['x_width_nm'] for r in rows], 120., atol=2.1)
            self.assertEqual([t['trial_pattern_key'] for t in trials], ['via60'])
            self.assertEqual(trials[0]['mode'], 'FORCED')
            self.assertIn('__via60__', Path(status['annotated_path']).name)
            self.assertTrue(Path(status['annotated_path']).is_file())
            self.assertEqual(status['source_algorithm'], 'V32_V27_Yakun_fusion')

    def test_relaxed_via_size_does_not_accept_blank_or_invent_edges(self):
        _, pool = analytic_vias()
        gray = np.full((360, 430), 180, np.uint8)
        with contextlib.redirect_stdout(io.StringIO()):
            rows, _, _, _ = sem._select_and_measure_vias_v19(gray, pool, 3., 'blank.png', 'before', '1', 'via60')
        self.assertEqual(rows, [])
        search, meta = sem._v32_via_search_scale([], gray.shape, 1., 'via60')
        self.assertEqual(search, 60.)
        self.assertEqual(meta['via_search_scale_source'], 'nominal_fallback')

    def test_slot_and_trench_are_not_capped_at_pattern_number_nm(self):
        # Inclusive raster slot is 17x21 px -> 255x315 nm at its half-gray edge;
        # trench 20px wide, 60px gap -> 200/600nm.
        gray = gradient_slots(40.)
        pool = sem._v32_slot_flatfield_candidates(gray, [])
        with contextlib.redirect_stdout(io.StringIO()):
            rows, selected, _, _, _ = sem._select_and_measure_bottom_slots_v19(
                gray, pool, 15., 'slot.png', 'before', '1', 'slot210')
        self.assertEqual(len(rows), 3)
        self.assertEqual(len(selected), 12)
        np.testing.assert_allclose([r['x_width_px'] for r in rows], 17., atol=.7)
        self.assertTrue(all(r['x_width_nm'] > 210 and r['y_height_nm'] > 210 for r in rows))
        rows, _, _, _ = sem._select_and_measure_trenches_v19(
            analytic_trenches(), [], 10., 'trench.png', 'before', '1', 'trench160')
        self.assertEqual(len(rows), 9)
        np.testing.assert_allclose([r['x_width_nm'] for r in rows], 200., atol=7.)
        np.testing.assert_allclose([r['tip_gap_y_nm'] for r in rows], 600., atol=7.)

    def test_fallbacks_keep_widened_size_limits_and_restore_settings(self):
        for pattern, names in (
            ('via60', ('V23_VIA_MIN_SPAN_FACTOR', 'V23_VIA_MAX_SPAN_FACTOR')),
            ('trench160', ('V20_TRENCH_GAP_FRAC_TOL', 'V20_TRENCH_WIDTH_FRAC_TOL')),
        ):
            original = [getattr(sem, name) for name in names]
            for level in (1, 2):
                knobs = sem._fallback_knobs_v25(pattern, level)
                values = sem._temporary_global_overrides_v25(knobs, lambda: [getattr(sem, name) for name in names])
                if pattern == 'via60':
                    self.assertLessEqual(values[0], original[0])
                else:
                    self.assertGreaterEqual(values[0], original[0])
                self.assertGreaterEqual(values[1], original[1])
                self.assertEqual([getattr(sem, name) for name in names], original)
